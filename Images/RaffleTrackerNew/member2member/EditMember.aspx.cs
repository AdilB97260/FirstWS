...
public partial class Church_EditChurch : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            DIST_USERS objDistUser = _provider.GetDistUserObj(UserSession.Inst.Member2MemberPK);
            if (objDistUser != null)
            {
                ...
                {
                    ltrBredCrumb.Text = string.Format("<div class='btn-group btn-breadcrumb'><a href='/member2member/dashboard.aspx' class='btn btn-primary'><i class='glyphicon glyphicon-home'></i></a><a href='/member2member/dashboard.aspx' class='btn btn-primary'>Home</a> <span class='btn' style='cursor:auto; color:#e4dfdf'>{0}</span></div>", "User");
                }
    ...
    protected void btnSave_click(object sernder, EventArgs e)
    {
        ...
        if (objDistUser != null)
        {
            ...
            if (result > 0)
            {
                ...
                //{
                //    Response.Redirect("/Church/MemberList.aspx");
                //}
                //else if (UserSession.Inst.UserType == "ADMIN")
                //{
                //    if (UserSession.Inst.Member2MemberPK > 0)
                //    {
                //        Response.Redirect("/Admin/Member2MemberList.aspx");
                //    }
                //    else
                //    {
    ...
    public static string EditTicket(int ticketFrom, int toTicket, int TotTicket, int distFkey, string distType)
    {
        ...
        bool canEdit = _provider.IsTicketDistributionOverlapping(Convert.ToInt32(ticketFrom), Convert.ToInt32(toTicket), UserSession.Inst.Member2MemberPK, distType);
    ...
    public static string ReDistTicket(int ticketFrom, int toTicket, int TotTicket, int distFkey)
    {
        ...
        bool isEdit = _provider.UpdateReDestTicket(ticketFrom.ToString(), toTicket.ToString(), TotTicket, Convert.ToInt32(distFkey), UserSession.Inst.Member2MemberPK, "MEMBER2MEMBER");
